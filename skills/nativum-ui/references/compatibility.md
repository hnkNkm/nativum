...
- `accent-color`
- `light-dark()`（カラートークン。`@supports (color: light-dark(#000, #fff))` ガード内で使用し、ガード外に静的なライト値フォールバックを宣言。未対応環境ではライト配色へフォールバック。ただしサーバーが `<html data-theme="dark">` を強制した場合は `:root[data-theme="dark"]` の静的ダーク値が `light-dark()` の有無に関わらず適用される。レンダリング例は `examples/settings-dark.html`）
